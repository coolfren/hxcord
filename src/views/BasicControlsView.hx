...
class BasicControlsView extends View {
    ...
    private function createTextCtrls(targetSizer:Sizer) {
        ...
        var textCtrl:TextCtrl = new TextCtrl(frame, "", TextCtrlStyle.MULTILINE);
        textCtrl.dragAcceptFiles(true);
        ...
        var button:Button = new Button(frame, "Send");
        button.bind(EventType.BUTTON, function(e:Event)
            {
                //clicked
                trace("CLICKED BUTTON (X)");
                trace("TEXT: " + textCtrl.value);
            });
